...
matrixA <- matrixA/100 + 1  # will need to use coordinates if use all sites in 1 model! or figure out distance b/w sites
matrixA
...
inits <- function() {
  list(y = y, alpha0=rnorm(1,-2,.4), alpha1=runif(1,1,2), s=sst, z=z)
}

# Error in setParameters(init.values[[i]], i) : Error in node y[1,1]
...
# Hi Jose,
#yeah, I know that problem. One thing that might improve the inits is to generate initial values for the activity centers outside of the inits function, and have the multinomial cell probabilities in the piece of code you quote above depend on those activity centers (assign unidentified records to the individual that lives closes to where the records were obtained). I have never tried if this helps, but is the only thing I can think of, off the top of my head.
#Cheers
#Rahel

#Latent variable is inconsistent with observed data
